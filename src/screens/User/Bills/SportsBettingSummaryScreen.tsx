...
const SportsBettingSummaryScreen
  = (props: Props) => {
    const {
    } = props;
    ...
    const makePurchase = async () => {
      try {
        setLoading(true)
        let payload = {
          "bet_id": form.provider,
          "customer_id": form.recipient,
          "customer_name": form.customer_name,
          "amount": form.amount,
          pin,
        }
        const res = await axiosExtended.post(`${routes.bills}/betting/payment`, payload);
        if (res.status === 200) {
    ...
    return (
      <SafeAreaView
        ...
        <ScrollView
          ...
          <InfoRow
            title='Amount:'
            value={`N${Utils.currencyFormat(parseInt(form.amount), 0)}`}
          />
        ...
        <ConfirmModal
          ...
          loading={loading}
          title={`Are you sure you want to make this purchase for N${Utils.currencyFormat(parseInt(form.amount), 0)}?`}
        />
        <ConfirmModal
          icon={
            <View style={{
          ...
          title="Congratulations, Sports betting bill payment was sucessful."
        />
      </SafeAreaView>
    );
  };
...
export default SportsBettingSummaryScreen
