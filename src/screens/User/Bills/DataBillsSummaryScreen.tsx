  ...
  = (props: Props) => {
    ...
    const makePurchase = async () => {
      try {
        ...
        const res = await axiosExtended.post(`${routes.bills}/data/payment`, payload);
        if (res.status === 200) {
    ...
    return (
      <SafeAreaView
        ...
        <ScrollView
          ...
          <InfoRow
            title='Amount:'
            value={`N${Utils.currencyFormat(parseInt(form.bouquet.amount), 0)}`}
          />
        ...
        <ConfirmModal
          ...
          loading={loading}
          title={`Are you sure you want to make this purchase for N${Utils.currencyFormat(parseInt(form.bouquet.amount), 0)}?`}
        />
        <ConfirmModal
          icon={
            <View style={{
          ...
          title="Congratulations, Data bill payment was sucessful."
        />
      </SafeAreaView>
    );
  };
...
export default DataBillsSummaryScreen
